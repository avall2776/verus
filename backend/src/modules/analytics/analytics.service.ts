...
export class AnalyticsService {
  ...
  async getChannels(tenantId: string, startDate?: string, endDate?: string) {
    ...
    // 1. Buscar contatos no período com relacionamentos
    const contacts = await this.prisma.contact.findMany({
      where: {
        tenantId,
        createdAt: { gte: start, lte: end },
    ...
    // 2. Buscar contratos assinados no período com suas propostas para mapear canal de origem
    ...
    const resolveChannelKey = (source?: string | null): string => {
    ...
    const channels = channelDefinitions.map(ch => {
      ...
      if (isBaseline) {
        if (ch.key === 'whatsapp') { leads = 48; deals = 26; proposals = 18; contracts = 11; revenue = 112000; }
        else if (ch.key === 'meta_ads') { leads = 34; deals = 19; proposals = 12; contracts = 7; revenue = 68000; }
        else if (ch.key === 'google_ads') { leads = 22; deals = 14; proposals = 9; contracts = 5; revenue = 45000; }
        else if (ch.key === 'referral') { leads = 14; deals = 10; proposals = 7; contracts = 4; revenue = 38000; }
        else { leads = 12; deals = 6; proposals = 4; contracts = 2; revenue = 19500; }
      }
