...
export class SupportAiService {
  ...
  async updateConfig(dto: UpdateSupportAiConfigDto) {
    await this.getConfig(); // Garante que o registro inicial exista
   ...
   */
  async handleTicketCreated(ticketId: string) {
    try {
      ...
      if (!ticket || ticket.isAiPaused) return;
...
5. Retorne a resposta em formato estruturado JSON.`;
  ...
  /**
   * Processa réplica de mensagem enviada pelo cliente no chamado
   */
  async handleIncomingClientMessage(ticketId: string, clientMessageContent: string, senderName: string) {
    try {
      const config = await this.getConfig();
      if (!config.isActive) return;
      ...
      // Se o chamado estiver fechado e o cliente enviou mensagem, reabre
